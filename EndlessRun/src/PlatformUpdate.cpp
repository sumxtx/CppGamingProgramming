...
void PlatformUpdate::update(float fps)
{
  if(m_Position.intersects(*m_PlayerPosition))
  {
    ...
    {
      m_PlayerPosition->left = m_Position.left - m_PlayerPosition->width;
    }
    else if(m_Position.contains(playerLeft))
    {
      m_PlayerPosition->left = m_Position.left + m_Position.width;
    }
    else if(m_Position.contains(playerHead))
    {
      m_PlayerPosition->top = m_Position.top + m_Position.height;
    }
  }
}
